...
- **JWT revocation**: None — tokens are valid until 7-day expiry. Blocklist / refresh-token rotation / `token_version` all deferred; `requireAuth` does zero DB reads by design, and no password-change or logout-everywhere feature exists to justify the per-request cost. Revisit when any of those triggers (or a known leak) lands; `token_version` on `users` is the intended variant
...
- **DB indexes**: Add to both `schema.sql` and `initializeDatabase()` as `CREATE INDEX IF NOT EXISTS` (no try/catch — `IF NOT EXISTS` is itself idempotent). Before adding, check the schema: SQLite auto-creates an index for every `UNIQUE` constraint on a non-PK column (e.g. `users.email`, `users.username`, `cooking_partners(user_id, partner_id)`) — don't duplicate. For columns that are only ever filtered alongside `user_id` (most `meals` filters), prefer a composite like `(user_id, status, created_at DESC)` over a single-column index that the planner won't pick
- **Cook together**: Users have usernames (set at registration, displayed at top of ProfilePage). One-directional cooking partners (max 10, `cooking_partners` table, managed on Profile page). When `partner_id` passed to generate, server merges both profiles via `mergeProfiles()` in `services/profile-merge.ts` (macros summed, restrictions/dislikes unioned, cuisines intersected, cook time min'd), macro overrides ignored for partner meals, AI gets `servings: 2` context. Meal stored only under generating user. `findUserByUsername()` excludes `password_hash` from SELECT.
